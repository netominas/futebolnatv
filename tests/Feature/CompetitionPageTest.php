...
class CompetitionPageTest extends TestCase
{
    ...
    public function test_competition_page_lists_upcoming_and_last_ten_games(): void
    {
        $competition = Competition::create(['wosti_id' => 1, 'name' => 'Liga Azul', 'slug' => 'liga-azul']);
        ...
        $this->get(route('competitions.index'))->assertOk()->assertSee('Liga Azul');
        $this->get($competition->publicUrl())->assertOk()->assertSee('Jogos da Liga Azul')->assertViewHas('pastFixtures', fn ($fixtures) => $fixtures->count() === 10);
        $this->get(route('fixtures.by-date', ['date' => $next->starts_at->format('Y-m-d')]))->assertSee($competition->publicUrl());
        $this->get('/sitemap.xml')->assertSee($competition->publicUrl());
    }
}
